...
func init() {
	android.RegisterModuleType("aosap_generator", GeneratorFactory)
}
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = aosapExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = aosapExpandVariables(ctx, dep_file)
	...
	cmd := aosapExpandVariables(ctx, String(g.properties.Cmd))
