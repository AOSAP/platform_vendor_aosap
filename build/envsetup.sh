function __print_aosap_functions_help() {
cat <<EOF
Additional AOSAP functions:
...
{
    if [ "$OUT" ] ; then
        ZIPPATH=`ls -tr "$OUT"/aosap-*.zip | tail -1`
        if [ ! -f $ZIPPATH ] ; then
            echo "Nothing to eat"
            return 1
        fi
        ...
        echo "Found device"
        if (adb shell getprop ro.aosap.device | grep -q "$AOSAP_BUILD"); then
            echo "Rebooting to sideload for install"
            adb reboot sideload-auto-reboot
            adb wait-for-sideload
            adb sideload $ZIPPATH
        else
            echo "The connected device does not appear to be $AOSAP_BUILD, run away!"
        fi
        return $?
    else
        echo "Nothing to eat"
...
function installboot()
{
    ...
    adb root
    adb wait-for-device-recovery
    if (adb shell getprop ro.aosap.device | grep -q "$AOSAP_BUILD");
    then
...
function dopush()
{
    ...
    if (adb shell getprop ro.aosap.device | grep -q "$AOSAP_BUILD") || [ "$FORCE_PUSH" = "true" ];
    then
...
function repopick() {
    T=$(gettop)
    $T/vendor/aosap/build/tools/repopick.py $@
}
...
function fixup_common_out_dir() {
    ...
    if [ ! -z $AOSAP_FIXUP_COMMON_OUT ]; then
